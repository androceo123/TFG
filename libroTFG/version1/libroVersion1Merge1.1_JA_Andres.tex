...
La implementación seleccionada para todas las corridas one-class del estudio es el siguiente pipeline en tres etapas:
\[
...
f(x)=\operatorname{sign}\big(w^\top z(x)-\rho\big).
\]
Las peticiones con $f(x) < 0$ se clasifican como anómalas. Esta formulación preserva el supuesto central del enfoque ---entrenar solo con tráfico normal--- y mantiene el costo de entrenamiento e inferencia en un rango compatible con el tamaño de los datasets utilizados \cite{sklearn_sgd_ocsvm,sklearn_kernel_approx}.
...
El detalle formal de cada métrica y de su cálculo exacto se encuentra en la sección de fundamentos matemáticos del Marco teórico.
