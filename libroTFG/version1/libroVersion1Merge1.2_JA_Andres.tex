...
\listoffigures
\clearpage
\chapter*{Tabla de símbolos y notación general}
\phantomsection
\addcontentsline{toc}{chapter}{Tabla de símbolos y notación general}
\label{sec:tabla-simbolos}
La siguiente tabla se ubica al comienzo del documento para facilitar la consulta de la notación usada en los capítulos teóricos, metodológicos y experimentales. Incluye símbolos de datasets, modelos, métricas predictivas y métricas operativas.

{\footnotesize
\renewcommand{\arraystretch}{1.1}
\begin{longtable}{@{}L{4.8cm}L{10.4cm}@{}}
\toprule
...
$t_{build}$ & tiempo de construcción o entrenamiento final del modelo \\
$t_{apply}$ & tiempo total de aplicación del flujo entrenado sobre el conjunto medido \\
$\mathrm{Throughput}$ & peticiones procesadas por segundo durante la medición operativa \\
\bottomrule
\end{longtable}
}
...
\textbf{Alcance de la medición.} Se distinguen tres momentos del flujo experimental. Primero, la búsqueda de hiperparámetros, cuando existe. Segundo, la construcción o entrenamiento final del modelo. Tercero, la aplicación del flujo ya entrenado sobre peticiones medidas en una prueba operativa o \emph{benchmark}. Esta última etapa no incluye entrenamiento ni búsqueda de hiperparámetros: mide la capacidad de procesamiento de peticiones por unidad de tiempo. Cuando la medición desagrega tiempos, la latencia total corresponde a extracción de características más inferencia; la latencia de inferencia corresponde solo al paso del vector de características por el modelo. En esta fundamentación se prioriza la definición conceptual de las métricas; la correspondencia con campos concretos de salida y archivos de resultados se documenta en la metodología y en los capítulos de resultados.
...
Este valor se interpreta junto con las latencias por petición, porque el tiempo total depende de cuántas peticiones fueron medidas y de si el flujo incluyó extracción de características, inferencia o ambas etapas. Por ese motivo, el tiempo de aplicación no debe leerse como una propiedad aislada del modelo, sino como una medición del flujo operativo definido para la evaluación.
...
\end{equation}
El pico aproximado, $\mathrm{RSS}_{pico}$, resume la mayor memoria residente observada durante la prueba. El tamaño del modelo se calcula como el espacio ocupado por la representación persistida del modelo, según la Ecuación~\eqref{eq:model-size}:
\begin{equation}
\label{eq:model-size}
\text{Tamaño del modelo}=\text{bytes de la representación persistida del modelo}.
\end{equation}
...
CPU/memoria/tamaño & Recursos consumidos durante la ejecución y tamaño en bytes de la representación persistida del modelo. & Costo de despliegue. \\
\bottomrule
\end{longtable}
...
SR-BH~2020 fue publicado en Harvard Dataverse y descrito en el artículo asociado como un dataset multietiqueta para clasificación de ataques web mediante CAPEC \cite{srbh_dataverse,srbh_paper}. Fue construido a partir de tráfico real recolectado durante doce días de julio de 2020 en un servidor WordPress instalado en una máquina virtual y expuesto a Internet. El entorno utilizó ModSecurity para Apache con Core Rule Set en modo \textit{Detection Only}; de ese modo, las peticiones quedaron registradas sin ser bloqueadas y luego los logs fueron revisados manual y semiautomáticamente para corregir o confirmar la asignación normal/ataque y la clasificación CAPEC correspondiente \cite{srbh_paper}.

El artículo del dataset reporta 907.814 peticiones HTTP: 525.195 normales y 382.619 anómalas. Cada registro original contiene 24 características y un conjunto de 13 etiquetas: una etiqueta asociada a la normalidad de la petición y 12 categorías CAPEC para ataques web \cite{srbh_paper}. Además, las peticiones maliciosas pueden activar más de una categoría CAPEC, por lo que SR-BH~2020 no se limita a una etiqueta binaria ni a una única clase de ataque. Esa propiedad lo convierte en el dataset central para evaluar la formulación multietiqueta y para derivar, con una regla explícita, una versión multiclase basada en etiqueta primaria.

El dataset está alineado con un caso de uso tipo WAF porque el registro se realiza a nivel de petición HTTP y conserva información directamente inspeccionable por un mecanismo de protección web. En la representación adoptada por este trabajo, esos datos se normalizan hacia campos comunes como método, URI, cuerpo y, cuando está disponible, encabezados, a partir de los cuales se recalculan las 25 características usadas por los modelos. Esta decisión permite comparar SR-BH~2020 con CSIC~2010 y CSIC/TorpEda~2012 bajo una misma representación, aunque el dataset original de SR-BH haya sido publicado con una codificación y un conjunto de variables propios.

La principal fortaleza de SR-BH~2020 es su mayor realismo frente a datasets sintéticos, porque proviene de un honeypot expuesto a Internet y conserva una anotación semántica por patrones CAPEC. Sin embargo, ese mismo origen impone límites de interpretación: el tráfico refleja el contexto de un servidor WordPress específico, una ventana temporal concreta y una configuración de captura basada en ModSecurity en modo de detección. Por tanto, los resultados obtenidos sobre este dataset no deben generalizarse automáticamente a todas las aplicaciones web, APIs, servicios REST/SOAP o despliegues productivos con patrones de tráfico diferentes. También debe considerarse el desbalance entre clases y etiquetas, ya que algunas categorías CAPEC tienen mucho menos soporte que otras y pueden afectar la estabilidad de métricas macro o por etiqueta.

\subsection{SR-BH 2020 como referencia de tráfico real}
\begin{itemize}[leftmargin=*]
  \item \textbf{Tamaño y composición}: contiene 907.814 peticiones HTTP, con 525.195 normales y 382.619 anómalas, lo que ofrece volumen suficiente para evaluar escalabilidad y efectos de desbalance \cite{srbh_paper}.
  \item \textbf{Campos y representación}: el dataset original incluye 24 características; en este trabajo se recalculan 25 variables interpretables desde método, URI, cuerpo y encabezados disponibles para mantener una representación común entre datasets.
  \item \textbf{Etiquetas CAPEC multietiqueta}: incorpora 13 etiquetas, incluyendo normalidad y 12 categorías CAPEC, lo que permite ir más allá del binario y evaluar clasificación semántica por patrones \cite{srbh_paper,capec_about}.
  \item \textbf{Tráfico real de honeypot}: reduce el riesgo de sobre-ajuste a patrones sintéticos y captura variabilidad observada en un servidor expuesto a Internet, aunque restringida al contexto de captura del honeypot \cite{srbh_paper}.
  \item \textbf{Pertinencia a WAF}: modela información inspeccionable por un WAF, principalmente campos de la petición HTTP y señales asociadas al contenido enviado \cite{owasp_waf,modsecurity_site}.
  \item \textbf{Reproducibilidad y límites}: al estar publicado con DOI persistente en Harvard Dataverse facilita citación y replicación; no obstante, su ventana temporal, su aplicación base y el soporte desigual de algunas etiquetas deben tratarse como amenazas a la validez externa \cite{srbh_dataverse,srbh_paper}.
\end{itemize}
