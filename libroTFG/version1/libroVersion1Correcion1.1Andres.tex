...
La comparación experimental se distribuye sobre tres fuentes de datos con distinta granularidad de etiquetado: CSIC 2010, útil principalmente para detección binaria; CSIC/TorpEda 2012, apto para detección binaria y clasificación multiclase; y SR-BH 2020, un dataset de tráfico real de honeypot con anotación CAPEC multietiqueta. Para preservar comparabilidad entre escenarios, la implementación normaliza los campos de entrada y deriva las variables \texttt{label\_binary}, \texttt{label\_multiclass} y \texttt{label\_multilabel} según la información disponible en cada dataset.
...
Un \textit{Web Application Firewall} (WAF) inspecciona conversaciones HTTP y aplica políticas para mitigar ataques comunes
como SQL Injection y XSS \cite{owasp_waf}. En la práctica, los WAF basados únicamente en firmas/reglas requieren mantenimiento continuo
y tienden a degradarse frente a variaciones de \textit{payload} o ataques novedosos. ModSecurity es un ejemplo de motor WAF ampliamente usado
para monitoreo y reglas \cite{modsecurity_site}.

Por otro lado, los enfoques basados en anomalías aprenden una representación del tráfico ``normal'' y detectan desviaciones,
lo cual es útil cuando la cobertura de ataques es incompleta o cambiante. En el dominio web, trabajos clásicos como Kruegel y Vigna
muestran que atributos estadísticos y de estructura de requests permiten puntuar anomalías \cite{kruegel_vigna_2003}.

El planteamiento adoptado toma como base un enfoque por anomalías y lo extiende con un dataset de tráfico HTTP real con etiquetas de ataque
estructuradas (CAPEC), habilitando comparaciones entre \textit{one-class} y enfoques supervisados multiclase/multietiqueta.

\section{Planteamiento del problema}
En entornos reales existe: (i) alta variabilidad del tráfico legítimo, (ii) fuerte desbalance entre normales y ataques,
y (iii) ataques emergentes que evolucionan. Un sistema útil debe:
\begin{itemize}[leftmargin=*]
  \item detectar anomalías cuando hay pocos datos rotulados (o cambian los ataques), y
  \item cuando hay etiquetas, clasificar el tipo/patrón para respuesta e investigación.
\end{itemize}
...
\section{HTTP y superficie de ataque}
Una petición HTTP contiene método, URI (path y query), cabeceras y, en algunos casos, cuerpo. Los ataques web tienden a manifestarse como
patrones de estructura y codificación (p.ej., percent-encoding), longitudes atípicas y presencia de tokens específicos en URI/body.
Los WAF típicamente inspeccionan esos campos para aplicar políticas \cite{owasp_waf,modsecurity_site}.

\section{WAF: enfoque por firmas vs anomalías}
\begin{itemize}[leftmargin=*]
  \item \textbf{Firmas/reglas}: alta precisión para ataques conocidos, pero requiere mantenimiento continuo y puede fallar ante variantes.
  \item \textbf{Anomalías}: aprende el soporte del tráfico normal y detecta desviaciones; puede detectar lo novedoso, pero puede elevar falsos positivos si el normal es muy diverso.
\end{itemize}
En el dominio web, se han propuesto enfoques de detección de anomalías basados en atributos del request (longitudes, distribución de caracteres, etc.) \cite{kruegel_vigna_2003}.

% ============================================================================
% CAPÍTULO 3
% ============================================================================
\chapter{Base matemática de los modelos}

Este capítulo presenta la formulación matemática de los modelos utilizados y fija una notación única
para que las variables de las ecuaciones correspondan de manera directa con la implementación.

\section{Notación y definición exacta de variables}

\subsection{Dataset, features y objetivos}
...
\subsection{Tabla de símbolos (resumen)}
{\footnotesize
...
En la implementación, una fila $x_i$ corresponde a un conjunto fijo de 25 features (longitudes, estructura, codificación, tokens y codificación one-hot del método). En SR-BH~2020, el dataset publicado incluye 24 features; en la versión adoptada para esta evaluación se recalcula el esquema completo y se agrega una feature derivada de headers (\texttt{req\_content\_length}) para unificar el conjunto en 25.
Las variables objetivo corresponden a:
\texttt{label\_binary} (binario),
\texttt{label\_multiclass} (multiclase) y
\texttt{label\_multilabel} (multietiqueta).
En SR-BH, \texttt{label\_multiclass} se deriva desde \texttt{label\_multilabel} mediante una política explícita basada en severidad. Para mantener trazabilidad, el dataset procesado puede acompañar esa variable con \texttt{label\_multiclass\_first} (auditoría) y \texttt{label\_primary\_severity} (puntaje asociado a la etiqueta primaria).

\section{Detector one-class escalable}
...
Una forma general de expresar la decisión del modelo es:
...
puede interpretarse como una puntuación de normalidad. Si el valor queda del lado aceptado por la frontera, la petición se considera normal. Si queda del lado opuesto, se considera anómala. Por eso, el modelo no solo produce una etiqueta final, sino también una noción de distancia respecto al comportamiento aprendido.

\subsection{Frontera de decisión y anomalías}
...
\subsection{Caso binario y función sigmoide}
...
representada por un vector de características $x$, calcula un puntaje lineal:
\[
z=w^\top x+b,
\]
...
entre 0 y 1, se aplica la \textit{función sigmoide}:
\[
...
\subsection{Función de pérdida binaria}
...
$p_i=P(y=1\mid x_i)$, su forma regularizada es:
\[
\mathcal{L}(w,b)=
-\frac{1}{n}\sum_{i=1}^n
...
\subsection{Extensión multiclase mediante softmax}
...
probabilidades mediante la \textit{función softmax}:
\[
...
La pérdida asociada al caso multiclase es la \textit{entropía cruzada}:
\[
\mathcal{L}=
...
\section{Clasificación multietiqueta con One-vs-Rest (OvR)}
...
One-vs-Rest resuelve el problema multietiqueta entrenando un clasificador binario por etiqueta. Para cada etiqueta $\ell\in\{1,\dots,L\}$, el modelo aprende si esa etiqueta está presente o ausente:
\[
P(y_\ell=1\mid x)=\sigma(w_\ell^\top x+b_\ell).
\]

Después, cada probabilidad se convierte en una decisión binaria mediante un umbral $\tau_\ell$:
\[
\hat{y}_{\ell}=\ind\big[P(y_\ell=1\mid x)\ge \tau_\ell\big].
\]

La idea puede leerse con un ejemplo. Si las etiquetas posibles fueran \texttt{SQL Injection}, \texttt{Path Traversal} y \texttt{Command Injection}, OvR entrenaría tres clasificadores: uno pregunta si la petición es o no \texttt{SQL Injection}, otro si es o no \texttt{Path Traversal}, y otro si es o no \texttt{Command Injection}. Como las decisiones son independientes por etiqueta, la salida final puede contener una, varias o ninguna etiqueta.
...
La Figura~\ref{fig:ovr} resume este flujo. El vector $x$ entra a varios clasificadores binarios; cada clasificador produce una probabilidad asociada a una etiqueta; luego se aplican umbrales y se obtiene un vector de salida. Esta salida no es una única clase, sino una lista de decisiones sí/no, una por etiqueta.

\section{Métricas y base matemática}
...
multiclase y finalmente el caso multietiqueta. Dentro de cada bloque reutilizamos un mismo ejemplo numérico
a lo largo de las distintas métricas.

\subsection{Clasificación binaria}

\subsubsection{La matriz de confusión}
...
\textbf{Ejemplo.} Existían 20 ataques y se detectaron 16:
\[
...
\textbf{Ejemplo.} Con $\mathrm{Precision}=0{,}76$ y $\mathrm{Recall}=0{,}80$:
\[
F_1=\frac{2\cdot 0{,}76\cdot 0{,}80}{0{,}76+0{,}80}\approx 0{,}78.
\]

\subsubsection{Specificity (TNR), FPR y FNR}

Estas tres métricas son ``primas'' del recall, vistas desde la otra clase.
...
\textbf{Ejemplo.} Con $TN=75$, $FP=5$, $FN=4$, $TP=16$:
\[
...
\textbf{Ejemplo.} Con $\mathrm{TPR}=0{,}80$ y $\mathrm{TNR}=0{,}9375$:
\[
...
\subsubsection{ROC-AUC y PR-AUC}

Cuando el clasificador no entrega solo una etiqueta sino un \emph{score
...
$B$, $C$) sobre 100 instancias, con la siguiente matriz de confusión:
\[
\begin{array}{l|ccc|c}
...
A partir de esta matriz se obtienen los conteos por clase, que
...
\subsubsection{Accuracy multiclase}
...
\subsubsection{Balanced Accuracy multiclase}
...
$4/7\approx 0{,}571$ y $1/3\approx 0{,}333$:
\[
...
\subsubsection{Promedios micro, macro y weighted}
...
pregunta diferente. Para ilustrarlas calculamos primero precision,
recall y $F_1$ por clase a partir del ejemplo de referencia:
\[
\begin{array}{l|ccc}
...
\]
Cada \emph{instancia} pesa igual.

\emph{Ejemplo.} Con $\sum_k TP_k=90$, $\sum_k FP_k=10$ y $\sum_k FN_k=10$:
\[
\mathrm{Precision}_{micro}=\frac{90}{100}=0{,}90,\quad
\mathrm{Recall}_{micro}=\frac{90}{100}=0{,}90,\quad
F_{1,micro}=0{,}90.
\]
En problemas multiclase de etiqueta única las tres métricas micro
coinciden con la accuracy, como ocurre aquí ($0{,}90$).
...
\]
Cada \emph{clase} pesa igual. Penaliza el bajo desempeño en clases
minoritarias.
...
\textbf{Weighted.} Se calcula la métrica por clase y se promedia
ponderando por el soporte $s_k$ (cantidad de ejemplos verdaderos de la
clase $k$, con $\sum_k s_k = n$):
\[
F_{1,weighted}=\sum_{k=1}^{K}\frac{s_k}{n}F_{1,k}.
\]
Las clases mayoritarias pesan más. Es útil cuando se busca un valor
representativo del comportamiento global respetando el tamaño real de
cada clase.

\emph{Ejemplo.} Con soportes $s_A=90$, $s_B=7$, $s_C=3$:
\[
F_{1,weighted}=0{,}90\cdot 0{,}955+0{,}07\cdot 0{,}533+0{,}03\cdot 0{,}286\approx 0{,}906.
\]

\textbf{Comparación.} Sobre el mismo modelo:
\[
F_{1,micro}=0{,}90,\qquad F_{1,macro}\approx 0{,}59,\qquad F_{1,weighted}\approx 0{,}91.
\]
Las versiones micro y weighted dan valores parecidos y altos porque la
clase mayoritaria ($A$) domina el cálculo. La macro, en cambio, revela
que el modelo trata mal a las clases minoritarias: ese 0{,}59 es la
señal honesta de que $B$ y $C$ están lejos de funcionar bien.

\subsubsection{MCC multiclase}

El MCC se generaliza a $K$ clases mediante una fórmula equivalente
expresada sobre la matriz de confusión completa, manteniendo el rango y
la interpretación: $+1$ es perfecto, $0$ es aleatorio y los valores
negativos indican peor que el azar. Sigue siendo robusto frente al
desbalance.

\subsubsection{ROC-AUC y PR-AUC multiclase}
...
$\mathrm{AUC}_B=0{,}80$, $\mathrm{AUC}_C=0{,}60$. Entonces
\[
...
tener \emph{varias etiquetas a la vez} o ninguna. Por ejemplo, una
...
etiquetas ($L_1$, $L_2$, $L_3$), con verdad y predicción:
\[
...
\end{array}
\]
Hay $nL=9$ decisiones etiqueta-instancia en total. Computando por
etiqueta los conteos:
\[
\begin{array}{l|ccc}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Micro:} agrega todas las decisiones positivas de todas
  las etiquetas antes de calcular la métrica. Cada decisión
...
\textbf{Ejemplo.} Calculamos primero por etiqueta:
\[
\begin{array}{l|ccc}
...
aporta pocas decisiones positivas en términos absolutos.

\subsubsection{Jaccard Similarity}

\textbf{Fórmula (por instancia):}
\[
...
\emph{Samples (promedio sobre instancias).}
\[
...
\subsubsection{ROC-AUC y PR-AUC multietiqueta}
...
Mide el tiempo usado por el ajuste final del modelo:
\[
t_{build}=t_{fin\_fit}-t_{inicio\_fit}.
\]

Esta métrica permite comparar cuánto cuesta construir cada modelo una vez fijada su configuración.

\paragraph{Tiempo de aplicación.}
Mide el tiempo total empleado por el benchmark para procesar un conjunto de peticiones:
\[
t_{apply}=\mathrm{wall\_time\_seconds}.
\]

Este valor se interpreta junto con las latencias por petición, porque el tiempo total del benchmark puede depender de cuántas peticiones fueron medidas.

\paragraph{Throughput.}
Indica cuántas peticiones se procesan por segundo:
\[
\mathrm{Throughput}=\frac{\mathrm{successful\_requests}}{\mathrm{wall\_time\_seconds}}.
\]

Un throughput mayor sugiere mayor capacidad de procesamiento, aunque debe leerse junto con latencia y estabilidad.

\paragraph{Latencia.}
La latencia mide cuánto tarda una petición individual en atravesar el pipeline de extracción de características e inferencia. Se reportan percentiles como P50, P95 y P99. P95 es especialmente útil porque indica que el 95\% de las peticiones medidas terminó por debajo de ese tiempo; por tanto, ayuda a observar el comportamiento de cola y no solo el promedio.

\paragraph{Estabilidad del benchmark.}
La tasa de solicitudes fallidas durante el benchmark no describe errores de clasificación, sino fallos de ejecución o medición durante la prueba operativa:
\[
\mathrm{Tasa\ de\ fallos}=\frac{\mathrm{failures}}{\mathrm{total\_requests\_measured}}.
\]

\paragraph{Uso de CPU, memoria y tamaño de modelo.}
El uso aproximado de CPU puede estimarse como:
\[
\mathrm{CPU\ util\ \%}\approx 100\cdot \frac{\Delta(\mathrm{user}+\mathrm{system})}{\mathrm{wall\_time\_seconds}}.
\]

La memoria y el tamaño del modelo permiten evaluar si la solución es liviana o costosa para despliegues futuros. En conjunto, estas métricas ayudan a decidir si un modelo con buen rendimiento predictivo también es razonable desde el punto de vista operativo.

\subsection{Diccionario resumido de métricas retenidas}
...
\begin{itemize}[leftmargin=*]
  ...
  \item labels: \texttt{label\_binary}, \texttt{label\_multiclass}, \texttt{label\_multilabel}.
\end{itemize}
