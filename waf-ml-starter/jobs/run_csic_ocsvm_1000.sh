...
#SBATCH --job-name=ocsvm_csic_1000_gpu
#SBATCH --output=slurm-csic-1000-%j.out
#SBATCH --gres=gpu:1
...
mkdir -p resultsOptimo/csic/oneclass_test1000 data/tmp

# Crea un parquet temporal de 1000 filas del dataset CSIC procesado.
# Si existe label_binary, intenta muestrear de forma estratificada para mantener la proporción normal/anómalo.
...
out = Path("data/tmp/csic_features_1000.parquet")
...
PYTHONPATH=src srun .venv/bin/python src/waf_ml/optimo/ocsvmOptimo.py \
  ...
  --data data/tmp/csic_features_1000.parquet \
  ...
  --tune-n-jobs 4 \
  --require-gpu \
  --out resultsOptimo/csic/oneclass_test1000/ocsvm.joblib \
  --metrics-out resultsOptimo/csic/oneclass_test1000/metrics.json \
  --pred-out resultsOptimo/csic/oneclass_test1000/pred.csv \
  --tune-results-out resultsOptimo/csic/oneclass_test1000/search.csv \
  --fi-kind shap \
  --fi-out resultsOptimo/csic/oneclass_test1000/feature_importance_shap.csv \
  --fi-max-rows 200 \
  --shap-background-rows 50 \
  --benchmark-mode auto \
  --benchmark-max-rows 500 \
  --benchmark-out resultsOptimo/csic/oneclass_test1000/benchmark.json
