#!/usr/bin/env bash
#SBATCH --job-name=corrida3mejores
#SBATCH --ntasks=1
...
on_error() {
  ...
  echo "  cat ${LOG_PREFIX:-${PROJECT_DIR:-/home_data/aroman/TFG/waf-ml-starter}/corrida3mejoresResultadosPosibles/multiclase/logs/corrida3mejoresResultadosPosibles_${RUN_DATASET:-both}_${SLURM_JOB_ID:-JOBID}}.err"
  exit "${code}"
}
trap on_error ERR
...
TRAIN_PY="${TRAIN_PY:-${PROJECT_DIR}/corrida3mejoresResultadosPosibles.py}"
CORRIDA3_RESULT_DIR="${CORRIDA3_RESULT_DIR:-${DEFINITIVO_DIR:-${PROJECT_DIR}/corrida3mejoresResultadosPosibles}}"
...
# Preprocesamiento/labels: mismas 84 features optimizadas para ambos; labels según dataset.
...
mkdir -p "${LOG_DIR}"
LOG_PREFIX="${LOG_DIR}/corrida3mejoresResultadosPosibles_${RUN_DATASET}_${SLURM_JOB_ID:-manual}"
exec > >(tee -a "${LOG_PREFIX}.out") 2> >(tee -a "${LOG_PREFIX}.err" >&2)

if [[ ! -f "${TRAIN_PY}" ]]; then
  echo "[ERROR] No existe TRAIN_PY=${TRAIN_PY}" >&2
  echo "Copiá corrida3mejoresResultadosPosibles.py a PROJECT_DIR o exportá TRAIN_PY=/ruta/al/corrida3mejoresResultadosPosibles.py" >&2
  exit 2
fi
...
[INFO] Features: optimized84 intra-request, iguales para ambos datasets
...
================================================================
[OK] $(date '+%F %T') corrida3mejoresResultadosPosibles multiclase finalizado.
[OUT_ROOT] ${CORRIDA3_RESULT_DIR}
[ARTIFACTS PRINCIPALES]
  comparison_csv: ${CORRIDA3_RESULT_DIR}/multiclase/model_comparison_multiclass_corrida3mejoresResultadosPosibles.csv
  comparison_json: ${CORRIDA3_RESULT_DIR}/multiclase/model_comparison_multiclass_corrida3mejoresResultadosPosibles.json
  manifest: ${CORRIDA3_RESULT_DIR}/manifest_corrida3mejoresResultadosPosibles.json
  torpeda_metrics: ${CORRIDA3_RESULT_DIR}/multiclase/torpeda/metrics_multiclass_histgb_optimized84.json
  harvard_metrics: ${CORRIDA3_RESULT_DIR}/multiclase/harvard/metrics_multiclass_histgb_optimized84.json
...
find "${CORRIDA3_RESULT_DIR}" -maxdepth 3 -type f \( -name 'metrics_multiclass_histgb_optimized84.json' -o -name 'metrics_flat.csv' -o -name 'feature_importance.csv' -o -name 'operational_metrics.json' -o -name 'model_comparison_multiclass_corrida3mejoresResultadosPosibles.csv' -o -name 'feature_importance_per_class.csv' -o -name 'feature_importance_binary_normal_vs_attack.csv' -o -name 'selected_model_summary.txt' \) -print | sort
