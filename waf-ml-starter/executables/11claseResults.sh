...
on_error() {
  ...
  echo "  cat ${LOG_PREFIX:-${PROJECT_DIR:-/home_data/aroman/TFG/waf-ml-starter}/11claseResults/multiclase/logs/11claseResults_${RUN_DATASET:-both}_${SLURM_JOB_ID:-JOBID}}.err"
  exit "${code}"
}
trap on_error ERR
...
TRAIN_PY="${TRAIN_PY:-${PROJECT_DIR}/11claseResults.py}"
RESULT_DIR="${RESULT_DIR:-${DEFINITIVO_DIR:-${PROJECT_DIR}/11claseResults}}"
...
# Harvard queda en modo top11-operational-groups por defecto: conserva el top 14,
# agrupa CAPEC-272/CAPEC-274 como HTTPProtocolManipulation y CAPEC-153/CAPEC-194
# como DataManipulation, y descarta el resto antes de extraer features. TorpEda queda igual.
TORPEDA_ONLY_COMMON_LABELS="${TORPEDA_ONLY_COMMON_LABELS:-1}"
TORPEDA_MIN_CLASS_COUNT="${TORPEDA_MIN_CLASS_COUNT:-0}"
HARVARD_LABEL_MODE="${HARVARD_LABEL_MODE:-top11-operational-groups}"
...
mkdir -p "${RESULT_DIR}/multiclase/torpeda/logs" "${RESULT_DIR}/multiclase/harvard/logs"
LOG_DIR="${RESULT_DIR}/multiclase/logs"
mkdir -p "${LOG_DIR}"
LOG_PREFIX="${LOG_DIR}/11claseResults_${RUN_DATASET}_${SLURM_JOB_ID:-manual}"
exec > >(tee -a "${LOG_PREFIX}.out") 2> >(tee -a "${LOG_PREFIX}.err" >&2)

if [[ ! -f "${TRAIN_PY}" ]]; then
  echo "[ERROR] No existe TRAIN_PY=${TRAIN_PY}" >&2
  echo "Copiá 11claseResults.py a PROJECT_DIR o exportá TRAIN_PY=/ruta/al/11claseResults.py" >&2
  exit 2
fi
...
[INFO] Harvard label mode=${HARVARD_LABEL_MODE} (top11-operational-groups => top14 + agrupación operativa en 11 clases finales)
...
================================================================
[OK] $(date '+%F %T') 11claseResults multiclase finalizado.
[OUT_ROOT] ${RESULT_DIR}
[ARTIFACTS PRINCIPALES]
  comparison_csv: ${RESULT_DIR}/multiclase/model_comparison_multiclass_11claseResults.csv
  comparison_json: ${RESULT_DIR}/multiclase/model_comparison_multiclass_11claseResults.json
  manifest: ${RESULT_DIR}/manifest_11claseResults.json
  torpeda_metrics: ${RESULT_DIR}/multiclase/torpeda/metrics_multiclass_histgb_useful58_operational.json
  harvard_metrics: ${RESULT_DIR}/multiclase/harvard/metrics_multiclass_histgb_useful58_operational.json
...
find "${RESULT_DIR}" -maxdepth 3 -type f \( -name 'metrics_multiclass_histgb_useful58_operational.json' -o -name 'metrics_flat.csv' -o -name 'feature_importance.csv' -o -name 'operational_metrics.json' -o -name 'model_comparison_multiclass_11claseResults.csv' -o -name 'feature_importance_per_class.csv' -o -name 'feature_importance_binary_normal_vs_attack.csv' -o -name 'selected_model_summary.txt' \) -print | sort
