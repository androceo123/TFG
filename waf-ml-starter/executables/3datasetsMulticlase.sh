...
on_error() {
  ...
  echo "  cat ${LOG_PREFIX:-${PROJECT_DIR:-/home_data/aroman/TFG/waf-ml-starter}/3datasetsMulticlase/multiclase/logs/3datasetsMulticlase_${RUN_DATASET:-all}_${SLURM_JOB_ID:-JOBID}}.err"
  exit "${code}"
}
trap on_error ERR
...
TRAIN_PY="${TRAIN_PY:-${PROJECT_DIR}/3datasetsMulticlase.py}"
RESULT_DIR="${RESULT_DIR:-${DEFINITIVO_DIR:-${PROJECT_DIR}/3datasetsMulticlase}}"
...
# Preprocesamiento/labels: mismas 58 features útiles; labels según dataset.
...
mkdir -p "${LOG_DIR}"
LOG_PREFIX="${LOG_DIR}/3datasetsMulticlase_${RUN_DATASET}_${SLURM_JOB_ID:-manual}"
exec > >(tee -a "${LOG_PREFIX}.out") 2> >(tee -a "${LOG_PREFIX}.err" >&2)

if [[ ! -f "${TRAIN_PY}" ]]; then
  echo "[ERROR] No existe TRAIN_PY=${TRAIN_PY}" >&2
  echo "Copiá 3datasetsMulticlase.py a PROJECT_DIR o exportá TRAIN_PY=/ruta/al/3datasetsMulticlase.py" >&2
  exit 2
fi
...
[INFO] Features: useful58 intra-request, iguales para los 3 datasets
...
================================================================
[OK] $(date '+%F %T') 3datasetsMulticlase multiclase finalizado.
[OUT_ROOT] ${RESULT_DIR}
[ARTIFACTS PRINCIPALES]
  comparison_csv: ${RESULT_DIR}/multiclase/model_comparison_multiclass_3datasetsMulticlase.csv
  comparison_json: ${RESULT_DIR}/multiclase/model_comparison_multiclass_3datasetsMulticlase.json
  manifest: ${RESULT_DIR}/manifest_3datasetsMulticlase.json
  ...
  egipcios_metrics: ${RESULT_DIR}/multiclase/egipcios/metrics_multiclass_histgb_useful58_request10.json
