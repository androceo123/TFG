...
on_error() {
  ...
  echo "  cat ${LOG_PREFIX:-${PROJECT_DIR:-/home_data/aroman/TFG/waf-ml-starter}/10claseResults/multiclase/logs/10claseResults_${RUN_DATASET:-both}_${SLURM_JOB_ID:-JOBID}}.err"
  exit "${code}"
}
trap on_error ERR
...
TRAIN_PY="${TRAIN_PY:-${PROJECT_DIR}/10claseResults.py}"
RESULT_DIR="${RESULT_DIR:-${DEFINITIVO_DIR:-${PROJECT_DIR}/10claseResults}}"
...
# como HARVARD-RequestManipulation, y descarta el resto antes de extraer features. TorpEda queda igual.
...
mkdir -p "${LOG_DIR}"
LOG_PREFIX="${LOG_DIR}/10claseResults_${RUN_DATASET}_${SLURM_JOB_ID:-manual}"
exec > >(tee -a "${LOG_PREFIX}.out") 2> >(tee -a "${LOG_PREFIX}.err" >&2)

if [[ ! -f "${TRAIN_PY}" ]]; then
  echo "[ERROR] No existe TRAIN_PY=${TRAIN_PY}" >&2
  echo "Copiá 10claseResults.py a PROJECT_DIR o exportá TRAIN_PY=/ruta/al/10claseResults.py" >&2
  exit 2
fi
...
================================================================
[OK] $(date '+%F %T') 10claseResults multiclase finalizado.
[OUT_ROOT] ${RESULT_DIR}
[ARTIFACTS PRINCIPALES]
  comparison_csv: ${RESULT_DIR}/multiclase/model_comparison_multiclass_10claseResults.csv
  comparison_json: ${RESULT_DIR}/multiclase/model_comparison_multiclass_10claseResults.json
  manifest: ${RESULT_DIR}/manifest_10claseResults.json
  torpeda_metrics: ${RESULT_DIR}/multiclase/torpeda/metrics_multiclass_histgb_useful58_request10.json
  harvard_metrics: ${RESULT_DIR}/multiclase/harvard/metrics_multiclass_histgb_useful58_request10.json
