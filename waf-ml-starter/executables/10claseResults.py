...
# -*- coding: utf-8 -*-
"""
10claseResults multiclase WAF-ML para TorpEda y Harvard/SR-BH.
...
- Guarda TODO bajo 10claseResults/multiclase/{torpeda,harvard}.
...
DEFAULT_RESULT_DIR_NAME = "10claseResults"
...
def train_eval_one_dataset(args: argparse.Namespace, dataset: str) -> Dict[str, Any]:
    ...
    print("================================================================", flush=True)
    print(f"[START] {dataset} 10claseResults multiclase HistGradientBoosting @ {_now_iso()}", flush=True)
...
def make_summary_text(m: Dict[str, Any]) -> str:
    ...
    lc = m.get("loss_curve") or {}
    lines = [
        "BEST_MODEL_10CLASERESULTS_MULTICLASE",
...
def write_aggregate_outputs(args: argparse.Namespace, payloads: Sequence[Dict[str, Any]]) -> None:
    ...
    df.to_csv(root / "model_comparison_multiclass_10claseResults.csv", index=False)
    _save_json({
        "created_at": _now_iso(),
        "task": "multiclass",
        "model_family": "hist_gradient_boosting",
        ...
        "rows": rows,
        "artifacts": {
            "comparison_csv": str(root / "model_comparison_multiclass_10claseResults.csv"),
            "comparison_json": str(root / "model_comparison_multiclass_10claseResults.json"),
        },
    }, root / "model_comparison_multiclass_10claseResults.json")
    _save_json({
        "created_at": _now_iso(),
    ...
    }, Path(args.definitivo_dir) / "manifest_10claseResults.json")
    print(f"[OK] Aggregate comparison: {root / 'model_comparison_multiclass_10claseResults.csv'}", flush=True)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="10claseResults multiclase: HistGradientBoosting con 58 features intra-request útiles; Harvard top 14 agrupado en 10 clases operativas por defecto")
    ...
    ap.add_argument("--definitivo-dir", "--result-dir", dest="definitivo_dir", default=None, help="Default: PROJECT_DIR/10claseResults")
    ...
    ap.add_argument("--output-dir", default=None, help="Solo para --dataset torpeda/harvard: sobreescribe 10claseResults/multiclase/{dataset}.")
    ...
    if args.dataset == "both" and args.output_dir:
        raise SystemExit("--output-dir solo se usa con --dataset torpeda o --dataset harvard; con both se usa 10claseResults/multiclase/{dataset}.")
