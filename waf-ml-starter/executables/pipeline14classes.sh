#!/usr/bin/env bash
#SBATCH --job-name=pipeline14classes
#SBATCH --ntasks=1
...
on_error() {
  ...
  echo "  cat ${LOG_PREFIX:-${PROJECT_DIR:-/home_data/aroman/TFG/waf-ml-starter}/pipeline14classesResult/multiclase/logs/pipeline14classes_${RUN_DATASET:-both}_${SLURM_JOB_ID:-JOBID}}.err"
  exit "${code}"
}
trap on_error ERR
...
TRAIN_PY="${TRAIN_PY:-${PROJECT_DIR}/pipeline14classes.py}"
PIPELINE14CLASSES_RESULT_DIR="${PIPELINE14CLASSES_RESULT_DIR:-${DEFINITIVO_DIR:-${PROJECT_DIR}/pipeline14classesResult}}"
DEFINITIVO_DIR="${PIPELINE14CLASSES_RESULT_DIR}"  # compatibilidad con ejecuciones antiguas que exportaban DEFINITIVO_DIR
...
mkdir -p "${PIPELINE14CLASSES_RESULT_DIR}/multiclase/torpeda/logs" "${PIPELINE14CLASSES_RESULT_DIR}/multiclase/harvard/logs"
LOG_DIR="${PIPELINE14CLASSES_RESULT_DIR}/multiclase/logs"
mkdir -p "${LOG_DIR}"
LOG_PREFIX="${LOG_DIR}/pipeline14classes_${RUN_DATASET}_${SLURM_JOB_ID:-manual}"
exec > >(tee -a "${LOG_PREFIX}.out") 2> >(tee -a "${LOG_PREFIX}.err" >&2)

if [[ ! -f "${TRAIN_PY}" ]]; then
  echo "[ERROR] No existe TRAIN_PY=${TRAIN_PY}" >&2
  echo "Copiá pipeline14classes.py a PROJECT_DIR o exportá TRAIN_PY=/ruta/al/pipeline14classes.py" >&2
  exit 2
fi
...
[INFO] PIPELINE14CLASSES_RESULT_DIR=${PIPELINE14CLASSES_RESULT_DIR}
...
ARGS=(
  ...
  --result-dir "${PIPELINE14CLASSES_RESULT_DIR}"
...
================================================================
[OK] $(date '+%F %T') pipeline14classes multiclase finalizado.
[OUT_ROOT] ${PIPELINE14CLASSES_RESULT_DIR}
[ARTIFACTS PRINCIPALES]
  comparison_csv: ${PIPELINE14CLASSES_RESULT_DIR}/multiclase/model_comparison_multiclass_pipeline14classes.csv
  comparison_json: ${PIPELINE14CLASSES_RESULT_DIR}/multiclase/model_comparison_multiclass_pipeline14classes.json
  manifest: ${PIPELINE14CLASSES_RESULT_DIR}/manifest_pipeline14classes.json
  torpeda_metrics: ${PIPELINE14CLASSES_RESULT_DIR}/multiclase/torpeda/metrics_multiclass_histgb_fixed30.json
  harvard_metrics: ${PIPELINE14CLASSES_RESULT_DIR}/multiclase/harvard/metrics_multiclass_histgb_fixed30.json
================================================================
EOF

find "${PIPELINE14CLASSES_RESULT_DIR}" -maxdepth 3 -type f \( -name 'metrics_multiclass_histgb_fixed30.json' -o -name 'metrics_flat.csv' -o -name 'feature_importance.csv' -o -name 'operational_metrics.json' -o -name 'model_comparison_multiclass_pipeline14classes.csv' -o -name 'selected_model_summary.txt' \) -print | sort
