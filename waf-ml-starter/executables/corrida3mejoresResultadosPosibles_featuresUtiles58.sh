...
# Submit base:
#   sbatch corrida3mejoresResultadosPosibles_featuresUtiles58.sh
# Si tu cuenta requiere partición explícita:
#   sbatch -p normal corrida3mejoresResultadosPosibles_featuresUtiles58.sh
# Si Arandu sólo acepta la cola normal con GRES GPU, podés pedirla desde CLI,
# aunque HistGradientBoostingClassifier no la use:
#   sbatch -p normal --gres=gpu:1 corrida3mejoresResultadosPosibles_featuresUtiles58.sh
...
on_error() {
  ...
  echo "  cat ${LOG_PREFIX:-${PROJECT_DIR:-/home_data/aroman/TFG/waf-ml-starter}/corrida3mejoresResultadosPosibles_featuresUtiles58/multiclase/logs/corrida3mejoresResultadosPosibles_featuresUtiles58_${RUN_DATASET:-both}_${SLURM_JOB_ID:-JOBID}}.err"
  exit "${code}"
}
trap on_error ERR
...
TRAIN_PY="${TRAIN_PY:-${PROJECT_DIR}/corrida3mejoresResultadosPosibles_featuresUtiles58.py}"
CORRIDA3_RESULT_DIR="${CORRIDA3_RESULT_DIR:-${DEFINITIVO_DIR:-${PROJECT_DIR}/corrida3mejoresResultadosPosibles_featuresUtiles58}}"
DEFINITIVO_DIR="${CORRIDA3_RESULT_DIR}"  # compatibilidad con ejecuciones antiguas que exportaban DEFINITIVO_DIR
...
# Preprocesamiento/labels: mismas 58 features útiles para ambos; labels según dataset.
# Harvard queda en modo top14-combo por defecto: sólo conserva las 14 combinaciones CAPEC
# más frecuentes y descarta el resto antes de extraer features. TorpEda queda igual.
TORPEDA_ONLY_COMMON_LABELS="${TORPEDA_ONLY_COMMON_LABELS:-1}"
TORPEDA_MIN_CLASS_COUNT="${TORPEDA_MIN_CLASS_COUNT:-0}"
HARVARD_LABEL_MODE="${HARVARD_LABEL_MODE:-top14-combo}"
...
mkdir -p "${CORRIDA3_RESULT_DIR}/multiclase/torpeda/logs" "${CORRIDA3_RESULT_DIR}/multiclase/harvard/logs"
LOG_DIR="${CORRIDA3_RESULT_DIR}/multiclase/logs"
mkdir -p "${LOG_DIR}"
LOG_PREFIX="${LOG_DIR}/corrida3mejoresResultadosPosibles_featuresUtiles58_${RUN_DATASET}_${SLURM_JOB_ID:-manual}"
exec > >(tee -a "${LOG_PREFIX}.out") 2> >(tee -a "${LOG_PREFIX}.err" >&2)

if [[ ! -f "${TRAIN_PY}" ]]; then
  echo "[ERROR] No existe TRAIN_PY=${TRAIN_PY}" >&2
  echo "Copiá corrida3mejoresResultadosPosibles_featuresUtiles58.py a PROJECT_DIR o exportá TRAIN_PY=/ruta/al/corrida3mejoresResultadosPosibles_featuresUtiles58.py" >&2
  exit 2
fi
...
[INFO] CORRIDA3_RESULT_DIR=${CORRIDA3_RESULT_DIR}
...
[INFO] Harvard label mode=${HARVARD_LABEL_MODE} (top14-combo => ignora combinaciones fuera del top 14)
[INFO] Features: useful58 intra-request, iguales para ambos datasets
...
ARGS=(
  ...
  --result-dir "${CORRIDA3_RESULT_DIR}"
...
================================================================
[OK] $(date '+%F %T') corrida3mejoresResultadosPosibles_featuresUtiles58 multiclase finalizado.
[OUT_ROOT] ${CORRIDA3_RESULT_DIR}
[ARTIFACTS PRINCIPALES]
  comparison_csv: ${CORRIDA3_RESULT_DIR}/multiclase/model_comparison_multiclass_corrida3mejoresResultadosPosibles_featuresUtiles58.csv
  comparison_json: ${CORRIDA3_RESULT_DIR}/multiclase/model_comparison_multiclass_corrida3mejoresResultadosPosibles_featuresUtiles58.json
  manifest: ${CORRIDA3_RESULT_DIR}/manifest_corrida3mejoresResultadosPosibles_featuresUtiles58.json
  torpeda_metrics: ${CORRIDA3_RESULT_DIR}/multiclase/torpeda/metrics_multiclass_histgb_useful58.json
  harvard_metrics: ${CORRIDA3_RESULT_DIR}/multiclase/harvard/metrics_multiclass_histgb_useful58.json
  torpeda_fi_per_class: ${CORRIDA3_RESULT_DIR}/multiclase/torpeda/feature_importance_per_class.csv
  harvard_fi_per_class: ${CORRIDA3_RESULT_DIR}/multiclase/harvard/feature_importance_per_class.csv
================================================================
EOF

find "${CORRIDA3_RESULT_DIR}" -maxdepth 3 -type f \( -name 'metrics_multiclass_histgb_useful58.json' -o -name 'metrics_flat.csv' -o -name 'feature_importance.csv' -o -name 'operational_metrics.json' -o -name 'model_comparison_multiclass_corrida3mejoresResultadosPosibles_featuresUtiles58.csv' -o -name 'feature_importance_per_class.csv' -o -name 'feature_importance_binary_normal_vs_attack.csv' -o -name 'selected_model_summary.txt' \) -print | sort
