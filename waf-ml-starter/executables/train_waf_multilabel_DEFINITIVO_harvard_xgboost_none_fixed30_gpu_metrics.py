...
# -*- coding: utf-8 -*-
"""
Pipeline DEFINITIVO multietiqueta WAF-ML para Harvard/SR-BH con XGBoost/none.

- Procesa el dataset crudo Harvard/SR-BH desde CSV/TSV/GZ.
- Extrae exactamente el vector fijo de 30 features numéricas intra-request.
- Entrena un clasificador binario One-vs-Rest por etiqueta CAPEC usando XGBoost sin ponderación de clases (weight_mode=none).
- Usa GPU/CUDA cuando XGBoost y Slurm exponen una GPU; si CUDA falla, reintenta CPU por etiqueta.
- Optimiza umbrales por etiqueta con validación interna para maximizar F1.
- Evalúa en test externo y guarda métricas funcionales y no funcionales, importancia de características y artefactos.
- Guarda resultados bajo DEFINITIVO/multietiqueta/multiquetaSalidaXG por defecto.
"""
...
DEFAULT_RESULTS_DIRNAME = "DEFINITIVO"
...
DEFAULT_MULTILABEL_MODEL_NAME = "xgboost_none_ovr_fixed30"


def _dataset_out_dir(args: argparse.Namespace) -> Path:
    if args.output_dir:
        return Path(args.output_dir)
    return Path(args.definitivo_dir) / "multietiqueta" / "multiquetaSalidaXG"
...
def compute_multilabel_metrics(
    ...
    binary_metrics = {
        ...
        "balanced_accuracy": _safe_float(balanced_accuracy_score(yt_bin, yp_bin)),
...
def _labelset_from_binary_row(row: np.ndarray, labels: Sequence[str]) -> str:
    idx = np.flatnonzero(np.asarray(row).astype(int) > 0)
    if len(idx) == 0:
        return "NORMAL"
    return "|".join(str(labels[int(i)]) for i in idx)


def _ordered_labels_from_indices(indices: Iterable[int], labels: Sequence[str]) -> List[str]:
    idx_set = {int(i) for i in indices}
    return [str(labels[i]) for i in range(len(labels)) if i in idx_set]


def _row_match_scores(n_true: int, n_pred: int, n_correct: int) -> Dict[str, float]:
    """Métricas por request para explicar casos tipo: tenía 3 etiquetas, acertó 2."""
    precision = float(n_correct / n_pred) if n_pred > 0 else (1.0 if n_true == 0 else 0.0)
    recall = float(n_correct / n_true) if n_true > 0 else (1.0 if n_pred == 0 else 0.0)
    if precision + recall > 0:
        f1 = float(2.0 * precision * recall / (precision + recall))
    else:
        f1 = 0.0
    union = n_true + n_pred - n_correct
    jaccard = float(n_correct / union) if union > 0 else 1.0
    return {
...
def write_multilabel_audit_outputs(
    Y_true: np.ndarray,
    Y_pred: np.ndarray,
    scores: np.ndarray,
    thresholds: np.ndarray,
    ...
    out_dir: Path,
    *,
    prediction_audit_df: Optional[pd.DataFrame] = None,
) -> Dict[str, Any]:
    """Guarda matrices de confusión y auditoría request-level para multietiqueta.

    Archivos clave:
    - confusion_matrix_by_label_*.csv/json: matriz 2x2 por etiqueta.
    - confusion_matrix_labelset.csv: pares conjunto-real vs conjunto-predicho.
    - request_match_by_true_and_correct_count.csv: conteo de 0/1/2/3 aciertos dentro de N etiquetas reales.
    - request_level_multilabel_match.csv: auditoría por request.
    """
    del scores, thresholds  # Scores/umbrales quedan en test_predictions y thresholds_validation.
    out_dir.mkdir(parents=True, exist_ok=True)
    Y_true = np.asarray(Y_true).astype(int)
    ...
    # 1) Matriz de confusión multilabel por etiqueta: cada etiqueta es un problema binario.
    if n_labels:
        mcm = multilabel_confusion_matrix(Y_true, Y_pred, labels=list(range(len(labels))))
    else:
        mcm = np.zeros((0, 2, 2), dtype=int)
    long_rows: List[Dict[str, Any]] = []
    wide_rows: List[Dict[str, Any]] = []
    json_labels: List[Dict[str, Any]] = []
    for j, lab in enumerate(labels):
        tn, fp, fn, tp = [int(x) for x in mcm[j].ravel()]
        wide_rows.append({
            "label": lab,
        ...
        json_labels.append({
            "label": lab,
            "label_index": int(j),
            "matrix_rows_actual_cols_predicted": [[tn, fp], [fn, tp]],
            "tn": tn,
            "fp": fp,
            "fn": fn,
            "tp": tp,
        })
    cm_long_path = out_dir / "confusion_matrix_by_label_long.csv"
    cm_wide_path = out_dir / "confusion_matrix_by_label_wide.csv"
    cm_json_path = out_dir / "confusion_matrix_by_label.json"
    pd.DataFrame(long_rows).to_csv(cm_long_path, index=False)
    pd.DataFrame(wide_rows).to_csv(cm_wide_path, index=False)
    _save_json({"created_at": _now_iso(), "n_labels": len(labels), "labels": json_labels}, cm_json_path)
    paths["confusion_matrix_by_label_long"] = str(cm_long_path)
    paths["confusion_matrix_by_label_wide"] = str(cm_wide_path)
    paths["confusion_matrix_by_label_json"] = str(cm_json_path)

    # 2) Normal vs ataque derivado: útil para comparar con una matriz clásica binaria.
    yt_bin = (Y_true.sum(axis=1) > 0).astype(int)
    yp_bin = (Y_pred.sum(axis=1) > 0).astype(int)
    cm_bin = confusion_matrix(yt_bin, yp_bin, labels=[0, 1])
    bin_rows: List[Dict[str, Any]] = []
    for ai, actual_name in enumerate(["NORMAL", "ATTACK"]):
        for pi, pred_name in enumerate(["NORMAL", "ATTACK"]):
            bin_rows.append({"actual": actual_name, "predicted": pred_name, "count": int(cm_bin[ai, pi])})
    bin_csv_path = out_dir / "confusion_matrix_binary_normal_vs_attack.csv"
    bin_json_path = out_dir / "confusion_matrix_binary_normal_vs_attack.json"
    pd.DataFrame(bin_rows).to_csv(bin_csv_path, index=False)
    _save_json({
        "created_at": _now_iso(),
        "labels_order": ["NORMAL", "ATTACK"],
        "matrix_rows_actual_cols_predicted": cm_bin.astype(int).tolist(),
    }, bin_json_path)
    paths["confusion_matrix_binary_normal_vs_attack_csv"] = str(bin_csv_path)
    paths["confusion_matrix_binary_normal_vs_attack_json"] = str(bin_json_path)

    # 3) Auditoría por request: cuántas etiquetas reales había y cuántas acertó.
    audit_rows: List[Dict[str, Any]] = []
    actual_sets: List[str] = []
    pred_sets: List[str] = []
    for r in range(n_rows):
        true_idx = set(int(i) for i in np.flatnonzero(Y_true[r] > 0))
        pred_idx = set(int(i) for i in np.flatnonzero(Y_pred[r] > 0))
        correct_idx = true_idx & pred_idx
        missed_idx = true_idx - pred_idx
        extra_idx = pred_idx - true_idx
        n_true = len(true_idx)
        n_pred = len(pred_idx)
        n_correct = len(correct_idx)
        actual_set = _labelset_from_binary_row(Y_true[r], labels)
        pred_set = _labelset_from_binary_row(Y_pred[r], labels)
        actual_sets.append(actual_set)
        pred_sets.append(pred_set)
        score_cols = _row_match_scores(n_true, n_pred, n_correct)
        audit_rows.append({
            "request_pos": int(r),
            "true_labelset": actual_set,
            "pred_labelset": pred_set,
            "n_true_labels": int(n_true),
            "n_pred_labels": int(n_pred),
            "n_correct_labels": int(n_correct),
            "n_missed_labels": int(len(missed_idx)),
            "n_extra_labels": int(len(extra_idx)),
            "exact_match": bool(true_idx == pred_idx),
            "all_true_labels_found": bool(len(missed_idx) == 0),
            "no_extra_labels": bool(len(extra_idx) == 0),
            "partial_match": bool(n_correct > 0 and true_idx != pred_idx),
            "correct_labels_json": json.dumps(_ordered_labels_from_indices(correct_idx, labels), ensure_ascii=False),
            "missed_labels_json": json.dumps(_ordered_labels_from_indices(missed_idx, labels), ensure_ascii=False),
            "extra_labels_json": json.dumps(_ordered_labels_from_indices(extra_idx, labels), ensure_ascii=False),
            **score_cols,
        })
    audit_df = pd.DataFrame(audit_rows)

    request_csv_path = out_dir / "request_level_multilabel_match.csv"
    if prediction_audit_df is not None and not prediction_audit_df.empty:
        # Reutiliza sample_id/source_file/score_max y columnas por etiqueta de test_predictions.
        base = prediction_audit_df.reset_index(drop=True).copy()
        if len(base) == len(audit_df):
            for c in audit_df.columns:
                if c not in base.columns:
                    base[c] = audit_df[c].values
            audit_for_csv = base
        else:
            audit_for_csv = audit_df
    else:
        audit_for_csv = audit_df
    audit_for_csv.to_csv(request_csv_path, index=False)
    paths["request_level_multilabel_match"] = str(request_csv_path)

    if not audit_df.empty:
        by_true_correct = (
            audit_df.groupby(["n_true_labels", "n_correct_labels"], dropna=False)
            .size()
            .reset_index(name="count")
            .sort_values(["n_true_labels", "n_correct_labels"])
        )
        bucket_totals = by_true_correct.groupby("n_true_labels")["count"].transform("sum")
        by_true_correct["pct_within_true_label_count"] = by_true_correct["count"] / bucket_totals.replace(0, np.nan)
        by_true_correct["pct_of_test"] = by_true_correct["count"] / max(1, n_rows)
    else:
        by_true_correct = pd.DataFrame(columns=["n_true_labels", "n_correct_labels", "count", "pct_within_true_label_count", "pct_of_test"])
    by_true_correct_path = out_dir / "request_match_by_true_and_correct_count.csv"
    by_true_correct.to_csv(by_true_correct_path, index=False)
    paths["request_match_by_true_and_correct_count"] = str(by_true_correct_path)

    if not audit_df.empty:
        by_error = (
            audit_df.groupby(["n_true_labels", "n_pred_labels", "n_correct_labels", "n_missed_labels", "n_extra_labels"], dropna=False)
            .size()
            .reset_index(name="count")
            .sort_values("count", ascending=False)
        )
        by_error["pct_of_test"] = by_error["count"] / max(1, n_rows)
    else:
        by_error = pd.DataFrame(columns=["n_true_labels", "n_pred_labels", "n_correct_labels", "n_missed_labels", "n_extra_labels", "count", "pct_of_test"])
    by_error_path = out_dir / "request_match_error_distribution.csv"
    by_error.to_csv(by_error_path, index=False)
    paths["request_match_error_distribution"] = str(by_error_path)

    if not audit_df.empty:
        labelset_pairs = (
            audit_df.groupby(["true_labelset", "pred_labelset"], dropna=False)
            .agg(
                count=("request_pos", "size"),
                mean_row_jaccard=("row_jaccard", "mean"),
                mean_row_recall=("row_recall", "mean"),
                mean_row_precision=("row_precision", "mean"),
            )
            .reset_index()
            .sort_values("count", ascending=False)
        )
        labelset_pairs["pct_of_test"] = labelset_pairs["count"] / max(1, n_rows)
    else:
        labelset_pairs = pd.DataFrame(columns=["true_labelset", "pred_labelset", "count", "mean_row_jaccard", "mean_row_recall", "mean_row_precision", "pct_of_test"])
    labelset_csv_path = out_dir / "confusion_matrix_labelset.csv"
    labelset_pairs.to_csv(labelset_csv_path, index=False)
    paths["confusion_matrix_labelset"] = str(labelset_csv_path)

    # Wide labelset matrix solo si no explota en columnas; si no, guardamos una versión top para inspección.
    actual_unique = pd.Series(actual_sets).nunique(dropna=False) if actual_sets else 0
    pred_unique = pd.Series(pred_sets).nunique(dropna=False) if pred_sets else 0
    wide_limit_cells = 200_000
    if actual_unique * pred_unique <= wide_limit_cells:
        wide = pd.crosstab(pd.Series(actual_sets, name="actual_labelset"), pd.Series(pred_sets, name="pred_labelset"))
        labelset_wide_path = out_dir / "confusion_matrix_labelset_wide.csv"
        wide.to_csv(labelset_wide_path)
        paths["confusion_matrix_labelset_wide"] = str(labelset_wide_path)
    else:
        top_actual = set(pd.Series(actual_sets).value_counts().head(50).index.tolist())
        top_pred = set(pd.Series(pred_sets).value_counts().head(50).index.tolist())
        reduced_actual = [x if x in top_actual else "__OTHER_TRUE_LABELSETS__" for x in actual_sets]
        reduced_pred = [x if x in top_pred else "__OTHER_PRED_LABELSETS__" for x in pred_sets]
        wide = pd.crosstab(pd.Series(reduced_actual, name="actual_labelset"), pd.Series(reduced_pred, name="pred_labelset"))
        labelset_wide_path = out_dir / "confusion_matrix_labelset_wide_top50.csv"
        wide.to_csv(labelset_wide_path)
        paths["confusion_matrix_labelset_wide_top50"] = str(labelset_wide_path)

    exact_count = int(audit_df["exact_match"].sum()) if not audit_df.empty else 0
    partial_count = int(audit_df["partial_match"].sum()) if not audit_df.empty else 0
    any_correct_count = int((audit_df["n_correct_labels"] > 0).sum()) if not audit_df.empty else 0
    attack_df = audit_df[audit_df["n_true_labels"] > 0] if not audit_df.empty else audit_df
    normal_df = audit_df[audit_df["n_true_labels"] == 0] if not audit_df.empty else audit_df
    summary = {
        "test_rows": n_rows,
        "n_labels": len(labels),
        "exact_match_count": exact_count,
        "exact_match_rate": _safe_div(exact_count, n_rows),
        "partial_match_count": partial_count,
        "partial_match_rate": _safe_div(partial_count, n_rows),
        "any_correct_label_count": any_correct_count,
        "any_correct_label_rate": _safe_div(any_correct_count, n_rows),
        "all_true_labels_found_rate": _safe_float(audit_df["all_true_labels_found"].mean()) if not audit_df.empty else None,
        "no_extra_labels_rate": _safe_float(audit_df["no_extra_labels"].mean()) if not audit_df.empty else None,
        "mean_row_precision": _safe_float(audit_df["row_precision"].mean()) if not audit_df.empty else None,
        "mean_row_recall": _safe_float(audit_df["row_recall"].mean()) if not audit_df.empty else None,
        "mean_row_f1": _safe_float(audit_df["row_f1"].mean()) if not audit_df.empty else None,
        "mean_row_jaccard": _safe_float(audit_df["row_jaccard"].mean()) if not audit_df.empty else None,
        "attack_rows": int(len(attack_df)) if attack_df is not None else 0,
        "attack_exact_match_rate": _safe_float(attack_df["exact_match"].mean()) if attack_df is not None and len(attack_df) else None,
        "attack_any_correct_label_rate": _safe_float((attack_df["n_correct_labels"] > 0).mean()) if attack_df is not None and len(attack_df) else None,
        "attack_all_true_labels_found_rate": _safe_float(attack_df["all_true_labels_found"].mean()) if attack_df is not None and len(attack_df) else None,
        "normal_rows": int(len(normal_df)) if normal_df is not None else 0,
        "normal_exact_match_rate": _safe_float(normal_df["exact_match"].mean()) if normal_df is not None and len(normal_df) else None,
        "by_true_label_count_and_correct_count": by_true_correct.to_dict(orient="records"),
        "artifacts": paths,
    }
    summary_path = out_dir / "request_match_summary.json"
    _save_json(summary, summary_path)
    paths["request_match_summary"] = str(summary_path)
...
def measure_operational_metrics_multilabel(
    ...
    metrics = {
        "available": True,
        "measured_rows": int(n),
        "operational_failures": int(operational_failures),
        "operational_failure_rate": _safe_div(operational_failures, n),
        "successful_rows": int(n - operational_failures),
        ...
        "throughput_req_s_pipeline_online": _safe_div(n - operational_failures, online_pipeline_wall),
...
def flatten_metrics_for_comparison(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    rmm = m.get("request_match_metrics") or {}
    ...
    return {
        ...
        "request_exact_match_rate": rmm.get("exact_match_rate"),
        "request_partial_match_rate": rmm.get("partial_match_rate"),
        "request_any_correct_label_rate": rmm.get("any_correct_label_rate"),
        "request_mean_row_precision": rmm.get("mean_row_precision"),
        "request_mean_row_recall": rmm.get("mean_row_recall"),
        "request_mean_row_f1": rmm.get("mean_row_f1"),
        "request_mean_row_jaccard": rmm.get("mean_row_jaccard"),
        "attack_exact_match_rate": rmm.get("attack_exact_match_rate"),
        "attack_any_correct_label_rate": rmm.get("attack_any_correct_label_rate"),
...
def make_summary_text(m: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"request_partial_match_rate={rmm.get('partial_match_rate')}",
        f"request_any_correct_label_rate={rmm.get('any_correct_label_rate')}",
        f"request_mean_row_recall={rmm.get('mean_row_recall')}",
        f"request_mean_row_jaccard={rmm.get('mean_row_jaccard')}",
        f"attack_any_correct_label_rate={rmm.get('attack_any_correct_label_rate')}",
...
def write_book_metric_mapping(out_dir: Path) -> None:
    """Tabla de correspondencia con las métricas funcionales/no funcionales del libro."""
    rows = [
        {"grupo": "funcional_multietiqueta", "metrica_libro": "Exact match ratio / subset accuracy", "campo_salida": "test_metrics.exact_match_accuracy", "archivo_principal": "metrics_multilabel_harvard_xgboost_none_fixed30.json"},
        {"grupo": "funcional_multietiqueta", "metrica_libro": "Hamming loss", "campo_salida": "test_metrics.hamming_loss", "archivo_principal": "metrics_multilabel_harvard_xgboost_none_fixed30.json"},
        {"grupo": "funcional_multietiqueta", "metrica_libro": "Precision micro/macro/weighted/samples", "campo_salida": "test_metrics.precision_micro|macro|weighted|samples", "archivo_principal": "test_metrics_multilabel.json"},
        {"grupo": "funcional_multietiqueta", "metrica_libro": "Recall/TPR micro/macro/weighted/samples", "campo_salida": "test_metrics.recall_micro|macro|weighted|samples", "archivo_principal": "test_metrics_multilabel.json"},
        {"grupo": "funcional_multietiqueta", "metrica_libro": "F1 micro/macro/weighted/samples", "campo_salida": "test_metrics.f1_micro|macro|weighted|samples", "archivo_principal": "test_metrics_multilabel.json"},
        {"grupo": "funcional_multietiqueta", "metrica_libro": "Jaccard micro/macro/weighted/samples", "campo_salida": "test_metrics.jaccard_micro|macro|weighted|samples", "archivo_principal": "test_metrics_multilabel.json"},
        {"grupo": "funcional_multietiqueta", "metrica_libro": "ROC-AUC micro/macro/weighted", "campo_salida": "test_metrics.roc_auc_micro|macro|weighted", "archivo_principal": "test_metrics_multilabel.json"},
        {"grupo": "funcional_multietiqueta", "metrica_libro": "PR-AUC micro/macro/weighted", "campo_salida": "test_metrics.pr_auc_micro|macro|weighted", "archivo_principal": "test_metrics_multilabel.json"},
        {"grupo": "por_etiqueta", "metrica_libro": "TP, FP, FN, TN, precision, recall/TPR, specificity/TNR, FPR, FNR, F1, ROC-AUC, PR-AUC", "campo_salida": "per_label_metrics.csv", "archivo_principal": "per_label_metrics.csv"},
        {"grupo": "confusion_multietiqueta", "metrica_libro": "Matriz 2x2 por etiqueta: TN, FP, FN, TP", "campo_salida": "confusion_matrix_by_label_long.csv|wide.csv|json", "archivo_principal": "confusion_matrix_by_label_wide.csv"},
        {"grupo": "confusion_multietiqueta", "metrica_libro": "Matriz por conjunto de etiquetas real vs predicho", "campo_salida": "confusion_matrix_labelset.csv", "archivo_principal": "confusion_matrix_labelset.csv"},
        {"grupo": "auditoria_request", "metrica_libro": "Aciertos parciales por request: N etiquetas reales vs K etiquetas correctas", "campo_salida": "request_match_by_true_and_correct_count.csv y request_level_multilabel_match.csv", "archivo_principal": "request_match_by_true_and_correct_count.csv"},
        {"grupo": "binario_derivado", "metrica_libro": "Normal vs ataque: accuracy, balanced accuracy, precision, recall/TPR, specificity/TNR, FPR, FNR, F1, MCC, ROC-AUC, PR-AUC", "campo_salida": "test_metrics.binary_normal_vs_attack", "archivo_principal": "binary_normal_vs_attack_metrics.json"},
        {"grupo": "umbrales", "metrica_libro": "Umbrales por etiqueta optimizados en validación", "campo_salida": "thresholds_validation.csv", "archivo_principal": "thresholds_validation.csv"},
        {"grupo": "no_funcional_tiempo", "metrica_libro": "Tiempo de carga, extracción de features, selección/entrenamiento, refit final, inferencia batch y total", "campo_salida": "timing.*", "archivo_principal": "metrics_multilabel_harvard_xgboost_none_fixed30.json"},
        {"grupo": "no_funcional_operativo", "metrica_libro": "Latencia P50/P95/P99/media del pipeline", "campo_salida": "operational_metrics.latency_ms_pipeline_online_*", "archivo_principal": "operational_metrics.json"},
        {"grupo": "no_funcional_operativo", "metrica_libro": "Throughput req/s pipeline online, pipeline batch e inferencia batch", "campo_salida": "operational_metrics.throughput_req_s_*", "archivo_principal": "operational_metrics.json"},
        {"grupo": "no_funcional_operativo", "metrica_libro": "CPU time y utilización aproximada", "campo_salida": "operational_metrics.cpu_time_* y cpu_utilization_pipeline_online_percent_of_all_cores", "archivo_principal": "operational_metrics.json"},
        {"grupo": "no_funcional_operativo", "metrica_libro": "Memoria RSS pico/actual", "campo_salida": "resource_usage.peak_rss_mb|current_rss_mb y operational_metrics.peak_rss_mb|current_rss_mb", "archivo_principal": "metrics_multilabel_harvard_xgboost_none_fixed30.json"},
        {"grupo": "no_funcional_operativo", "metrica_libro": "Tamaño del modelo", "campo_salida": "resource_usage.model_size_bytes|model_size_mb", "archivo_principal": "metrics_multilabel_harvard_xgboost_none_fixed30.json"},
        {"grupo": "no_funcional_operativo", "metrica_libro": "Tasa de fallos de la prueba operativa", "campo_salida": "operational_metrics.operational_failure_rate", "archivo_principal": "operational_metrics.json"},
        {"grupo": "interpretabilidad", "metrica_libro": "Feature importance", "campo_salida": "feature_importance.csv y feature_importance_xgb_gain_aggregated.csv", "archivo_principal": "feature_importance.csv"},
    ]
    df = pd.DataFrame(rows)
    df.to_csv(out_dir / "book_metric_mapping.csv", index=False)
    _save_json({"created_at": _now_iso(), "rows": rows}, out_dir / "book_metric_mapping.json")
...
def _try_return_completed_run(out_dir: Path, args: argparse.Namespace) -> Optional[Dict[str, Any]]:
    ...
    if not _resume_enabled(args) or not bool(getattr(args, "resume_skip_complete", True)):
        return None
    metrics_path = out_dir / "metrics_multilabel_harvard_xgboost_none_fixed30.json"
    model_path = out_dir / "model_multilabel_harvard_xgboost_none_fixed30.joblib"
...
def train_eval_harvard_multilabel(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for r, idx0 in enumerate(test_idx):
        true_idx = set(int(j) for j, v in enumerate(Y_test[r]) if int(v) == 1)
        pred_idx = set(int(j) for j, v in enumerate(Y_pred[r]) if int(v) == 1)
        correct_idx = true_idx & pred_idx
        missed_idx = true_idx - pred_idx
        extra_idx = pred_idx - true_idx
        true_labs = _ordered_labels_from_indices(true_idx, target_labels)
        pred_labs = _ordered_labels_from_indices(pred_idx, target_labels)
        correct_labs = _ordered_labels_from_indices(correct_idx, target_labels)
        missed_labs = _ordered_labels_from_indices(missed_idx, target_labels)
        extra_labs = _ordered_labels_from_indices(extra_idx, target_labels)
        score_cols = _row_match_scores(len(true_idx), len(pred_idx), len(correct_idx))
        row: Dict[str, Any] = {
            "row_index": int(idx0),
            "sample_id": str(df_features.iloc[idx0].get("sample_id", idx0)),
            "source_file": str(df_features.iloc[idx0].get("source_file", "")),
            "true_labelset": _labelset_from_binary_row(Y_test[r], target_labels),
            "pred_labelset": _labelset_from_binary_row(Y_pred[r], target_labels),
            "y_true_labels_json": json.dumps(true_labs, ensure_ascii=False),
            "y_pred_labels_json": json.dumps(pred_labs, ensure_ascii=False),
            "correct_labels_json": json.dumps(correct_labs, ensure_ascii=False),
            "missed_labels_json": json.dumps(missed_labs, ensure_ascii=False),
            "extra_labels_json": json.dumps(extra_labs, ensure_ascii=False),
            "n_true_labels": int(len(true_idx)),
            "n_pred_labels": int(len(pred_idx)),
            "n_correct_labels": int(len(correct_idx)),
            "n_missed_labels": int(len(missed_idx)),
            "n_extra_labels": int(len(extra_idx)),
            "exact_match": bool(true_idx == pred_idx),
            "all_true_labels_found": bool(len(missed_idx) == 0),
            "no_extra_labels": bool(len(extra_idx) == 0),
            "partial_match": bool(len(correct_idx) > 0 and true_idx != pred_idx),
            **score_cols,
    ...
    audit_payload = write_multilabel_audit_outputs(
        Y_test,
        Y_pred,
        test_scores,
        thresholds,
        target_labels,
        out_dir,
        prediction_audit_df=pred_df,
    )
    audit_paths = dict(audit_payload.get("paths") or {})
    request_match_metrics = dict(audit_payload.get("summary") or {})
    ...
    metrics_payload: Dict[str, Any] = {
        ...
        "request_match_metrics": request_match_metrics,
        ...
        "artifacts": {
            ...
            "metrics": str(out_dir / "metrics_multilabel_harvard_xgboost_none_fixed30.json"),
            ...
            "book_metric_mapping_csv": str(out_dir / "book_metric_mapping.csv"),
            "book_metric_mapping_json": str(out_dir / "book_metric_mapping.json"),
            **audit_paths,
            **fi_paths,
        },
    }
    _save_json(metrics_payload, out_dir / "metrics_multilabel_harvard_xgboost_none_fixed30.json")
    ...
    write_book_metric_mapping(out_dir)
    ...
    print(
        ...
        f"f1_micro={test_metrics.get('f1_micro')} pr_auc_macro={test_metrics.get('pr_auc_macro')} "
        f"partial_match={request_match_metrics.get('partial_match_rate')} "
        f"attack_any_correct={request_match_metrics.get('attack_any_correct_label_rate')} "
        f"gpu_used={metrics_payload.get('gpu_used')}",
        flush=True,
    )
    ...
    for p in [
        processed_path,
        model_path,
        out_dir / "metrics_multilabel_harvard_xgboost_none_fixed30.json",
        ...
        out_dir / "request_level_multilabel_match.csv",
        out_dir / "request_match_by_true_and_correct_count.csv",
        out_dir / "request_match_summary.json",
        out_dir / "confusion_matrix_by_label_wide.csv",
        out_dir / "confusion_matrix_by_label_long.csv",
        out_dir / "confusion_matrix_labelset.csv",
        out_dir / "confusion_matrix_binary_normal_vs_attack.csv",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="DEFINITIVO multietiqueta Harvard/SR-BH: XGBoost/none OVR fixed30")
    ap.add_argument("--project-dir", default=DEFAULT_PROJECT_DIR)
    ap.add_argument("--definitivo-dir", default=None, help="Default: PROJECT_DIR/DEFINITIVO")
    ...
    ap.add_argument("--output-dir", default=None, help="Sobrescribe DEFINITIVO/multietiqueta/multiquetaSalidaXG.")
    ...
    ap.add_argument("--search-level", default="none", choices=["none", "standard", "max"], help="none=un modo; standard/max usan candidate-weight-modes.")
    ap.add_argument("--candidate-weight-modes", default="none", help="Modos separados por coma: none,sqrt,cuberoot,balanced. Para el definitivo se usa none.")
    ...
    ap.add_argument(
        ...
        default="xgboost",
        help=(
            "Backend del modelo. Para el definitivo se usa xgboost. "
            "También acepta lista separada por comas si se desea comparar manualmente."
        ),
    )
    ...
    ap.add_argument("--xgb-early-stopping-rounds", type=int, default=80)
