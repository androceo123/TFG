...
# -*- coding: utf-8 -*-
"""
Pipeline14classes multiclase WAF-ML para TorpEda y Harvard/SR-BH.

- Usa un único modelo fijado por los experimentos previos: HistGradientBoostingClassifier.
- Mantiene exactamente el mismo vector fijo de 30 features numéricas intra-request para ambos datasets.
- No hace búsqueda de candidatos ni tuning externo: solo entrena el modelo fijado y lo testea.
- Guarda TODO bajo pipeline14classesResult/multiclase/{torpeda,harvard}.
- En Harvard/SR-BH, por defecto usa clases de combinación exacta y conserva sólo el top 14
  de combinaciones CAPEC más frecuentes; el resto de combinaciones se ignora antes de extraer features.
- Calcula métricas predictivas, métricas operativas/performance y feature importance.
...
DEFAULT_RESULT_DIR_NAME = "pipeline14classesResult"
...
def make_srbh_labels(df: pd.DataFrame, schema: SRBHSchema, *, label_prefix: str, harvard_label_mode: str) -> pd.DataFrame:
    ...
    if _is_harvard_top14_combo_mode(mode):
...
def apply_label_filters(df: pd.DataFrame, args: argparse.Namespace, dataset: str, out_dir: Path) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if dataset == "harvard" and _is_harvard_top14_combo_mode(getattr(args, "harvard_label_mode", "")):
...
def compute_feature_importance(
    ...
    if args.fi_max_rows and args.fi_max_rows > 0 and len(X_test) > args.fi_max_rows:
        idx = rng.choice(np.arange(len(X_test)), size=int(args.fi_max_rows), replace=False)
        X_fi = X_test[idx]
        y_fi = y_test[idx]
        sampled = True
    else:
    ...
    if args.fi_n_repeats and args.fi_n_repeats > 0:
        ...
        perm_df["permutation_importance_mean"] = 0.0
        perm_df["permutation_importance_std"] = 0.0
        perm_df["permutation_error"] = "disabled_by_fi_n_repeats"
    ...
    uni_df = _univariate_eta2(X_fi, y_fi, features)
    uni_df.to_csv(out_dir / "feature_importance_univariate_eta2.csv", index=False)

    out = pd.DataFrame({"feature": list(features)})
    out = out.merge(perm_df, on="feature", how="left").merge(uni_df, on="feature", how="left")
    for c in ["permutation_importance_mean", "permutation_importance_std", "univariate_eta2"]:
        out[c] = pd.to_numeric(out.get(c, 0.0), errors="coerce").replace([np.inf, -np.inf], np.nan).fillna(0.0)

    # HistGradientBoostingClassifier no expone feature_importances_; usamos permutation como ranking principal.
    ...
    _save_json({
        ...
        "primary_method": str(out["importance_method"].iloc[0]) if not out.empty else None,
        "permutation_error": perm_error,
        "fi_scoring": args.fi_scoring,
        "fi_n_repeats": int(args.fi_n_repeats),
        "fi_n_jobs": int(args.fi_n_jobs),
        "fi_max_rows": int(args.fi_max_rows),
        "fi_rows_used": int(len(X_fi)),
        "fi_sampled": bool(sampled),
        "artifacts": {
    ...
    return {
        ...
        "feature_importance_permutation": str(out_dir / "feature_importance_permutation.csv"),
        "feature_importance_univariate_eta2": str(out_dir / "feature_importance_univariate_eta2.csv"),
        "feature_importance_metadata": str(out_dir / "feature_importance_metadata.json"),
    }
...
def train_eval_one_dataset(args: argparse.Namespace, dataset: str) -> Dict[str, Any]:
    ...
    print("================================================================", flush=True)
    print(f"[START] {dataset} pipeline14classes multiclase HistGradientBoosting @ {_now_iso()}", flush=True)
    ...
    processed_path = _save_processed(df, out_dir / "processed_fixed30.parquet")
    ...
    model_path = out_dir / "model_multiclass_histgb_fixed30.joblib"
    joblib.dump({
        "task": "multiclass",
        "dataset": dataset,
        "model": model,
    ...
    fi_paths = compute_feature_importance(model, X_test, y_test, FIXED_FEATURES, args, out_dir)
    ...
    metrics_payload: Dict[str, Any] = {
        ...
        "artifacts": {
            ...
            "metrics": str(out_dir / "metrics_multiclass_histgb_fixed30.json"),
    ...
    _save_json(metrics_payload, out_dir / "metrics_multiclass_histgb_fixed30.json")
    ...
    for p in [
        processed_path,
        model_path,
        out_dir / "metrics_multiclass_histgb_fixed30.json",
...
def make_summary_text(m: Dict[str, Any]) -> str:
    ...
    om = m.get("operational_metrics") or {}
    lines = [
        "BEST_MODEL_PIPELINE14CLASSES_MULTICLASE",
...
def write_aggregate_outputs(args: argparse.Namespace, payloads: Sequence[Dict[str, Any]]) -> None:
    ...
    df.to_csv(root / "model_comparison_multiclass_pipeline14classes.csv", index=False)
    _save_json({
        "created_at": _now_iso(),
        "task": "multiclass",
        "model_family": "hist_gradient_boosting",
        ...
        "rows": rows,
        "artifacts": {
            "comparison_csv": str(root / "model_comparison_multiclass_pipeline14classes.csv"),
            "comparison_json": str(root / "model_comparison_multiclass_pipeline14classes.json"),
        },
    }, root / "model_comparison_multiclass_pipeline14classes.json")
    _save_json({
        "created_at": _now_iso(),
    ...
    }, Path(args.definitivo_dir) / "manifest_pipeline14classes.json")
    print(f"[OK] Aggregate comparison: {root / 'model_comparison_multiclass_pipeline14classes.csv'}", flush=True)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="pipeline14classes multiclase: HistGradientBoosting fixed30 para TorpEda/Harvard; Harvard top14-combo por defecto")
    ...
    ap.add_argument("--definitivo-dir", "--result-dir", dest="definitivo_dir", default=None, help="Default: PROJECT_DIR/pipeline14classesResult")
    ...
    ap.add_argument("--output-dir", default=None, help="Solo para --dataset torpeda/harvard: sobreescribe pipeline14classesResult/multiclase/{dataset}.")
    ...
    ap.add_argument(
        "--harvard-label-mode",
        default="top14-combo",
        choices=["top14-combo", "top14-combos", "top14-combination", "top14-combinations", "exact-top14-combo", "exact-top14-combos", "optimized-family", "optimized_family", "optimized", "coarse", "coarse-family", "coarse_family", "family", "mapped", "attack-family", "attack_families", "native", "capec", "srbh", "legacy-common-anomalous", "legacy_common_anomalous"],
    )
    ...
    if args.dataset == "both" and args.output_dir:
        raise SystemExit("--output-dir solo se usa con --dataset torpeda o --dataset harvard; con both se usa pipeline14classesResult/multiclase/{dataset}.")
