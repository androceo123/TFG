...
# -*- coding: utf-8 -*-
"""
3datasetsMulticlase2 multiclase WAF-ML para TorpEda, Harvard/SR-BH y Egipcios/CSV.

- Usa HistGradientBoostingClassifier como modelo multiclase principal.
- Conserva las 58 features intra-request validadas de la corrida anterior y agrega sólo
  14 features manuales faltantes del enfoque del paper: estadísticas globales y señales
  específicas de SSTI, SSRF, command/code injection y XSS.
...
- Guarda TODO bajo 3datasetsMulticlase2/multiclase/{torpeda,harvard,egipcios}.
- En Harvard/SR-BH, por defecto conserva el top 14 de combinaciones CAPEC más frecuentes
  y aplica una taxonomía operativa de 10 clases: CAPEC-272/CAPEC-274/CAPEC-272+274
  y CAPEC-153/CAPEC-194 se unifican como RequestManipulation. El resto de combinaciones
  se ignora antes de extraer features.
...
- Calcula y grafica curvas por época/iteración de boosting: log-loss, MSE,
  RMSE y error de predicción en train y test/validación.
...
DEFAULT_RESULT_DIR_NAME = "3datasetsMulticlase2"
...
# De estas se conservan 58; luego se añaden 14 features manuales nuevas, para un total de 72.
...
# Bloque mínimo inspirado en las familias de features publicadas por el paper.
# No usa TF-IDF ni miles de n-grams: mantiene el pipeline numérico, denso e interpretable.
PAPER_GAP_FEATURES: List[str] = [
    ...
    # Patrones de las familias que no estaban representadas explícitamente.
...
FIXED_FEATURES += PAPER_GAP_FEATURES
ALL_CANDIDATE_FEATURES: List[str] = ALL_84_CANDIDATE_FEATURES + PAPER_GAP_FEATURES
...
FEATURE_POLICY_NAME = "useful72_wamm_manual_intra_request"
FEATURE_SET_VERSION = "useful72_wamm_manual_request_groups_v2"
...
def extract_fixed_features(method: str, uri: str, headers: Optional[Dict[str, str]] = None, body: Any = None) -> Dict[str, float]:
    """Devuelve las 72 features útiles en el orden FIXED_FEATURES."""
...
def _normalize_egipcios_label(raw: Any, *, label_prefix: str = "EGIPCIOS", normal_regex: str = EGIPCIOS_DEFAULT_NORMAL_REGEX) -> str:
    s = _safe_str(raw).strip()
    pref = _safe_str(label_prefix).strip() or "EGIPCIOS"
    if not s:
        return f"{pref}-UNKNOWN"
    try:
        if re.search(normal_regex or EGIPCIOS_DEFAULT_NORMAL_REGEX, s, flags=re.IGNORECASE):
            return "NORMAL"
    except re.error:
        if "normal" in s.lower() or s.strip().lower() in {"0", "000"}:
            return "NORMAL"
    clean = re.sub(r"^\s*\d{1,4}\s*[-_:]\s*", "", s).strip()
    low = clean.lower()
    family: Optional[str] = None
    for pattern, name in [
        (r"sql|sqli", "SQLi"),
        (r"cross\s*site|xss", "XSS"),
        (r"ldap", "LDAPi"),
        (r"xpath", "XPath"),
        (r"format\s*string", "FormatString"),
        (r"buffer|overflow", "BufferOverflow"),
        (r"server\s*side\s*include|\bssi\b", "SSI"),
        (r"crlf|response\s*splitting", "CRLFi"),
        (r"path\s*traversal|directory\s*traversal|file\s*inclusion", "PathTraversal"),
        (r"command\s*injection|os\s*command|cmd", "CommandInjection"),
        (r"code\s*injection", "CodeInjection"),
        (r"request\s*smuggling|http\s*request\s*smuggling", "HTTPRequestSmuggling"),
        (r"verb\s*tampering|method\s*tampering", "HTTPVerbTampering"),
        (r"protocol\s*manipulation", "ProtocolManipulation"),
        (r"scan|scanner|vulnerable\s*software", "VulnerabilityScanning"),
    ]:
        if re.search(pattern, low, flags=re.IGNORECASE):
            family = name
            break
    if family is None:
        canon = _canonical_attack_name(clean)
        family = _camel_safe_attack_name(canon)
    return f"{pref}-{family}"
...
def _write_run_config(args: argparse.Namespace, dataset: str, inputs: Sequence[str], out_dir: Path, extra: Dict[str, Any]) -> None:
    payload = {
        ...
        "feature_selection_rule": "58 features validadas de la corrida anterior + 14 features manuales faltantes del paper; sin TF-IDF y sin cambio de modelo",
        "args": vars(args),
        "environment": {
            "python": sys.version,
...
def train_eval_one_dataset(args: argparse.Namespace, dataset: str) -> Dict[str, Any]:
    ...
    print("================================================================", flush=True)
    print(f"[START] {dataset} 3datasetsMulticlase2 multiclase HistGradientBoosting @ {_now_iso()}", flush=True)
    ...
    processed_path = _save_processed(df, out_dir / "processed_useful72_wamm_manual_request10.parquet")
    ...
    model_path = out_dir / "model_multiclass_histgb_useful72_wamm_manual_request10.joblib"
    joblib.dump({
        "task": "multiclass",
        "dataset": dataset,
        "model": model,
    ...
    metrics_payload: Dict[str, Any] = {
        ...
        "task": "multiclass",
        "dataset": dataset,
        "dataset_label_mode": args.harvard_label_mode if dataset == "harvard" else ("egipcios_col2_canonicalized" if dataset == "egipcios" else "torpeda_native_common_names"),
        "feature_policy": FEATURE_POLICY_NAME,
        "feature_count": len(FIXED_FEATURES),
        ...
        "artifacts": {
            ...
            "metrics": str(out_dir / "metrics_multiclass_histgb_useful72_wamm_manual_request10.json"),
    ...
    _save_json(metrics_payload, out_dir / "metrics_multiclass_histgb_useful72_wamm_manual_request10.json")
    ...
    for p in [
        processed_path,
        model_path,
        out_dir / "metrics_multiclass_histgb_useful72_wamm_manual_request10.json",
...
def make_summary_text(m: Dict[str, Any]) -> str:
    ...
    lc = m.get("loss_curve") or {}
    lines = [
        "BEST_MODEL_3DATASETSMULTILABEL_MULTICLASE",
...
def write_aggregate_outputs(args: argparse.Namespace, payloads: Sequence[Dict[str, Any]]) -> None:
    ...
    df.to_csv(root / "model_comparison_multiclass_3datasetsMulticlase2.csv", index=False)
    _save_json({
        "created_at": _now_iso(),
        "task": "multiclass",
        "model_family": "hist_gradient_boosting",
        ...
        "rows": rows,
        "artifacts": {
            "comparison_csv": str(root / "model_comparison_multiclass_3datasetsMulticlase2.csv"),
            "comparison_json": str(root / "model_comparison_multiclass_3datasetsMulticlase2.json"),
        },
    }, root / "model_comparison_multiclass_3datasetsMulticlase2.json")
    _save_json({
        "created_at": _now_iso(),
    ...
    }, Path(args.definitivo_dir) / "manifest_3datasetsMulticlase2.json")
    print(f"[OK] Aggregate comparison: {root / 'model_comparison_multiclass_3datasetsMulticlase2.csv'}", flush=True)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="3datasetsMulticlase2 multiclase: HistGradientBoosting con 58 features validadas + 14 features manuales faltantes del paper; TorpEda + Harvard + Egipcios")
    ap.add_argument("--dataset", default="all", choices=["torpeda", "harvard", "egipcios", "both", "all", "three", "3datasets"], help="Dataset a correr. Default: all = torpeda+harvard+egipcios; both conserva torpeda+harvard.")
    ap.add_argument("--project-dir", default=DEFAULT_PROJECT_DIR)
    ap.add_argument("--definitivo-dir", "--result-dir", dest="definitivo_dir", default=None, help="Default: PROJECT_DIR/3datasetsMulticlase2")
    ...
    ap.add_argument("--output-dir", default=None, help="Solo para --dataset torpeda/harvard/egipcios: sobreescribe 3datasetsMulticlase2/multiclase/{dataset}.")
    ...
    if args.dataset in {"both", "all", "three", "3datasets"} and args.output_dir:
        raise SystemExit("--output-dir solo se usa con --dataset torpeda, harvard o egipcios; con both/all se usa 3datasetsMulticlase2/multiclase/{dataset}.")
