...
# -*- coding: utf-8 -*-
"""
3datasetsMulticlase multiclase WAF-ML para TorpEda, Harvard/SR-BH y Egipcios/CSV.
...
- Guarda TODO bajo 3datasetsMulticlase/multiclase/{torpeda,harvard,egipcios}.
...
DEFAULT_RESULT_DIR_NAME = "3datasetsMulticlase"
...
FEATURE_SET_VERSION = "useful58_request_groups_v1"
...
def train_eval_one_dataset(args: argparse.Namespace, dataset: str) -> Dict[str, Any]:
    ...
    print("================================================================", flush=True)
    print(f"[START] {dataset} 3datasetsMulticlase multiclase HistGradientBoosting @ {_now_iso()}", flush=True)
    ...
    processed_path = _save_processed(df, out_dir / "processed_useful58_request10.parquet")
    ...
    model_path = out_dir / "model_multiclass_histgb_useful58_request10.joblib"
    joblib.dump({
        "task": "multiclass",
        "dataset": dataset,
        "model": model,
    ...
    metrics_payload: Dict[str, Any] = {
        ...
        "artifacts": {
            ...
            "metrics": str(out_dir / "metrics_multiclass_histgb_useful58_request10.json"),
    ...
    _save_json(metrics_payload, out_dir / "metrics_multiclass_histgb_useful58_request10.json")
    ...
    for p in [
        processed_path,
        model_path,
        out_dir / "metrics_multiclass_histgb_useful58_request10.json",
...
def write_aggregate_outputs(args: argparse.Namespace, payloads: Sequence[Dict[str, Any]]) -> None:
    ...
    df.to_csv(root / "model_comparison_multiclass_3datasetsMulticlase.csv", index=False)
    _save_json({
        "created_at": _now_iso(),
        "task": "multiclass",
        "model_family": "hist_gradient_boosting",
        ...
        "rows": rows,
        "artifacts": {
            "comparison_csv": str(root / "model_comparison_multiclass_3datasetsMulticlase.csv"),
            "comparison_json": str(root / "model_comparison_multiclass_3datasetsMulticlase.json"),
        },
    }, root / "model_comparison_multiclass_3datasetsMulticlase.json")
    _save_json({
        "created_at": _now_iso(),
    ...
    }, Path(args.definitivo_dir) / "manifest_3datasetsMulticlase.json")
    print(f"[OK] Aggregate comparison: {root / 'model_comparison_multiclass_3datasetsMulticlase.csv'}", flush=True)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="3datasetsMulticlase multiclase: HistGradientBoosting con 58 features intra-request útiles; TorpEda + Harvard + Egipcios CSV raw-request")
    ...
    ap.add_argument("--definitivo-dir", "--result-dir", dest="definitivo_dir", default=None, help="Default: PROJECT_DIR/3datasetsMulticlase")
    ...
    ap.add_argument("--output-dir", default=None, help="Solo para --dataset torpeda/harvard/egipcios: sobreescribe 3datasetsMulticlase/multiclase/{dataset}.")
    ...
    if args.dataset in {"both", "all", "three", "3datasets"} and args.output_dir:
        raise SystemExit("--output-dir solo se usa con --dataset torpeda, harvard o egipcios; con both/all se usa 3datasetsMulticlase/multiclase/{dataset}.")
