...
# -*- coding: utf-8 -*-
"""
Corrida3MejoresResultadosPosibles multiclase WAF-ML para TorpEda y Harvard/SR-BH.

- Usa HistGradientBoostingClassifier como modelo multiclase principal.
- Reemplaza el vector compacto original de 30 variables por un set optimizado de 84 features
  numéricas intra-request, iguales para ambos datasets.
...
- Guarda TODO bajo corrida3mejoresResultadosPosibles/multiclase/{torpeda,harvard}.
...
DEFAULT_RESULT_DIR_NAME = "corrida3mejoresResultadosPosibles"

COMMON_ATTACKS = ["BufferOverflow", "CRLFi", "FormatString", "LDAPi", "SQLi", "SSI", "XPath", "XSS"]

# Set optimizado de 84 features numéricas, fijas e intra-request.
# Las 30 primeras son las del baseline; las 54 restantes agregan señales de método,
# protocolo/headers, estructura de parámetros/body, XPath/XML y scanning de rutas.
...
FIXED_FEATURES: List[str] = BASE_30_FEATURES + EXTENDED_54_FEATURES
FEATURE_POLICY_NAME = "optimized84_intra_request"
FEATURE_SET_VERSION = "corrida3_optimized84_v1"
...
def extract_fixed_features(method: str, uri: str, headers: Optional[Dict[str, str]] = None, body: Any = None) -> Dict[str, float]:
    """Devuelve las 84 features optimizadas en el orden FIXED_FEATURES."""
...
def train_eval_one_dataset(args: argparse.Namespace, dataset: str) -> Dict[str, Any]:
    ...
    print("================================================================", flush=True)
    print(f"[START] {dataset} corrida3mejoresResultadosPosibles multiclase HistGradientBoosting @ {_now_iso()}", flush=True)
    ...
    processed_path = _save_processed(df, out_dir / "processed_optimized84.parquet")
    ...
    model_path = out_dir / "model_multiclass_histgb_optimized84.joblib"
    joblib.dump({
        "task": "multiclass",
        "dataset": dataset,
        "model": model,
    ...
    metrics_payload: Dict[str, Any] = {
        ...
        "artifacts": {
            ...
            "metrics": str(out_dir / "metrics_multiclass_histgb_optimized84.json"),
    ...
    _save_json(metrics_payload, out_dir / "metrics_multiclass_histgb_optimized84.json")
    ...
    for p in [
        processed_path,
        model_path,
        out_dir / "metrics_multiclass_histgb_optimized84.json",
...
def write_aggregate_outputs(args: argparse.Namespace, payloads: Sequence[Dict[str, Any]]) -> None:
    ...
    df.to_csv(root / "model_comparison_multiclass_corrida3mejoresResultadosPosibles.csv", index=False)
    _save_json({
        "created_at": _now_iso(),
        "task": "multiclass",
        "model_family": "hist_gradient_boosting",
        ...
        "rows": rows,
        "artifacts": {
            "comparison_csv": str(root / "model_comparison_multiclass_corrida3mejoresResultadosPosibles.csv"),
            "comparison_json": str(root / "model_comparison_multiclass_corrida3mejoresResultadosPosibles.json"),
        },
    }, root / "model_comparison_multiclass_corrida3mejoresResultadosPosibles.json")
    _save_json({
        "created_at": _now_iso(),
    ...
    }, Path(args.definitivo_dir) / "manifest_corrida3mejoresResultadosPosibles.json")
    print(f"[OK] Aggregate comparison: {root / 'model_comparison_multiclass_corrida3mejoresResultadosPosibles.csv'}", flush=True)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="corrida3mejoresResultadosPosibles multiclase: HistGradientBoosting con 84 features intra-request optimizadas; Harvard top14-combo por defecto")
    ...
    ap.add_argument("--definitivo-dir", "--result-dir", dest="definitivo_dir", default=None, help="Default: PROJECT_DIR/corrida3mejoresResultadosPosibles")
    ...
    ap.add_argument("--output-dir", default=None, help="Solo para --dataset torpeda/harvard: sobreescribe corrida3mejoresResultadosPosibles/multiclase/{dataset}.")
    ...
    if args.dataset == "both" and args.output_dir:
        raise SystemExit("--output-dir solo se usa con --dataset torpeda o --dataset harvard; con both se usa corrida3mejoresResultadosPosibles/multiclase/{dataset}.")
